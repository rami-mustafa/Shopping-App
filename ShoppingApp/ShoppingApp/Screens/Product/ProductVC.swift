...
//    Birisi favori düğmesine tıkladığı zaman bana bilgi verecek
    func productFavorited(product: Product) {
...
extension ProductVC : UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        tableView.deselectRow(at: indexPath, animated: true)
        let selectedProduct = products[indexPath.section]  // Burada .row yerine .section kullanılmalı.
        
        print("Selected product ID:", selectedProduct.id)
        
        // ProductDetailVC'yi programatik olarak oluşturun ve ürünü geçirin.
        let vc = ProductDetailVC()
        vc.product = selectedProduct
        vc.modalTransitionStyle = .crossDissolve
        vc.modalPresentationStyle = .overCurrentContext
